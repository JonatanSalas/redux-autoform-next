export default class LayoutProvider {
    ...
    static canonizeLayout(schema) {
        ...
        if (!schema.hasOwnProperty('entities')) {
            ...
            return LayoutProvider.canonizeLayout({
                entities: [{
                    name: 'default',
                    fields: schema
                }]
            ...
            if (schema.hasOwnProperty('entities')) {
                schema['entities'] = LayoutProvider.canonizeArray(schema['entities']);

                schema['entities'].forEach(entity => {
                    if (entity.hasOwnProperty('fields')) {
                        entity['fields'] = LayoutProvider.canonizeArray(entity['fields']);
                    }

                    if (entity.hasOwnProperty('layouts')) {
                        entity['layouts'] = LayoutProvider.canonizeArray(entity['layouts']);

                        entity['layouts'].forEach(layout => {
                            if (layout.hasOwnProperty('fields')) {
                                layout['fields'] = LayoutProvider.canonizeArray(layout['fields']);
                            }

                            if (layout.hasOwnProperty('groups')) {
                                layout['groups'] = LayoutProvider.canonizeArray(layout['groups']);

                                layout['groups'].forEach(group => {
                                    if (group.hasOwnProperty('fields')) {
                                        group['fields'] = LayoutProvider.canonizeArray(group['fields']);
                                    }
                                });
                            }
                        });
    ...
    static canonizeArray(obj) {
        if (!obj) return obj; // this is so the canonizeLayout method doesn't have to check every property for undefined
    ...
    static validateFieldMetadata(metadata) {
        if (!metadata) {
            throw Error('metadata should not be null or undefined');
        }

        if (!metadata.hasOwnProperty('name')) {
            throw Error('metadata\'s "name" property is required');
        }

        if (!metadata.hasOwnProperty('type')) {
            throw Error('metadata\'s "type" property is required');
        }
    }
     ...
     * @param layout
     * @param callback
     * @external https://github.com/gearz-lab/react-metaform/blob/master/docs-md/LayoutProvider.md
     */
